(function () {
  ...
  function initAudioControls() {
    ...
    if (!sidebar || !article || sidebar.querySelector(".audio-side-tools")) return;

    var topTrack = document.createElement("div");
    topTrack.className = "audio-reading-track";
    topTrack.setAttribute("aria-hidden", "true");
    topTrack.innerHTML = '<span class="audio-reading-bar"></span>';
    document.body.appendChild(topTrack);

    var tools = document.createElement("section");
    tools.className = "audio-side-tools";
    tools.setAttribute("aria-label", "阅读进度");
    tools.innerHTML =
      '<div class="audio-side-tools__head"><span>阅读进度</span><output>0%</output></div>' +
      '<div class="audio-side-progress" role="progressbar" aria-label="当前文章阅读进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span></span></div>';
    sidebar.insertBefore(tools, sidebar.firstChild);

    var output = tools.querySelector("output");
    var sideBar = tools.querySelector(".audio-side-progress span");
    var progress = tools.querySelector(".audio-side-progress");
    var pageBar = topTrack.querySelector(".audio-reading-bar");
    var scheduled = false;
