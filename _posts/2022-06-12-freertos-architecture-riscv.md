...
这份图集把 FreeRTOS 从应用 API 一直画到 RISC-V 硬件层，重点放在几个平时看源码最容易断开的地方：TCB 和各类任务列表怎么关联，调度器怎样启动，Tick 中断如何进入，以及任务上下文最终在哪里保存和恢复。
...
下方是完整交互版。目录可以跳转章节，“上电 → 正常工作”部分可以逐步展开，模块速查表也支持关键字筛选。窄屏阅读时，建议点右上角链接单独打开。
