...
调 WAV 时我先确认两件事：这是 WAVE 容器；里面的样本是不是 PCM。大部分 WAV 的确装 PCM，但不能因为后缀是 `.wav` 就跳过 `fmt` 块。
...
然后核对 `fmt`，再按 chunk size 找 `data`，不要假定音频数据永远从第 44 字节开始。很多解析器只支持“最典型的 PCM WAV”，换一个带扩展块的文件就会暴露问题。
