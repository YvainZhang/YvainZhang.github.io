...
这几个概念原本是我分开记的，后来做系统问题时发现它们经常一起出现：中断改变执行上下文，多核引入并发，MMU 改变地址视角，Cache 又让“内存里的值”不一定等于 CPU 此刻看到的值。
...
如果问题表现为偶发、只在多核出现或 DMA 后数据不一致，不能只盯着业务代码。先确认上下文、共享数据保护、地址映射和 cache maintenance，再往下查。
