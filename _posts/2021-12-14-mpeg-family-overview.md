---
layout: post
title: "MPEG 标准谱系概览"
subtitle: "把 MP3、AAC、MP4、H.264 放回同一条标准演进主线"
date: 2021-12-14
author: Yvain Zhang
header-img: "img/post-bg-universe.jpg"
tags:
  - MPEG
  - 标准
  - 多媒体
---

很多音视频术语之所以显得零散，不是因为它们真的彼此无关，而是因为缺少一条标准演进主线。  
MP3、AAC、MP4、AVC / H.264 这些常见名词，很多都和 MPEG 谱系直接相关。

## 1. MPEG 到底是什么

MPEG 是 Moving Picture Experts Group 的缩写。它的重要性不在于“这是一个名字”，而在于它长期定义了大量音视频相关标准，包括：

- 音频压缩
- 视频压缩
- 多媒体系统
- 文件封装
- 内容描述

很多今天仍在广泛使用的格式，背后都有 MPEG 的影子。

## 2. 为什么要了解 MPEG 谱系

如果不把这些概念放回标准体系里看，容易出现一种状态：

- 知道 MP3
- 听过 AAC
- 见过 MP4
- 用过 H.264

但不知道它们彼此是什么关系。  
一旦不知道谱系，就很难在阅读标准、文档、FFmpeg 输出或播放器格式说明时建立整体感。

## 3. MPEG-1：最常被记住的是 MP3

MPEG-1 面向较早期、较低码率的数字音视频应用场景。今天回头看，它最著名的遗产之一就是：

- MPEG-1 Layer 3，也就是 MP3

MP3 的流行也说明了一个事实：很多标准真正被人记住的，不是标准号，而是它最终落成的工程产物。

## 4. MPEG-2：广播和更高质量媒体场景

MPEG-2 相比 MPEG-1 更偏向更高质量的视频和广播场景。它常见于：

- DVD
- 数字电视
- 广播系统

在音频体系里，AAC 也和 MPEG-2 有直接关联。  
所以 AAC 不是“凭空冒出来的新名字”，它是在 MPEG 音频标准演进中的重要一环。

## 5. MPEG-4：现代多媒体体系里最重要的一代

MPEG-4 非常关键，因为它不再只是讨论单纯压缩率，而是更强调：

- 更灵活的多媒体系统
- 网络友好性
- 更完整的媒体组织和应用场景支持

和工程实践最相关的几条高频关联包括：

- AAC 音频体系
- MP4 容器
- AVC / H.264

很多今天的播放器、点播文件和移动端媒体格式，都能在 MPEG-4 体系里找到位置。

## 6. MPEG-7 和 MPEG-21 为什么也值得知道

它们不像 MPEG-1/2/4 那样高频，但了解它们有助于建立标准视角。

### MPEG-7

更偏向媒体内容描述，例如：

- 标注
- 分类
- 检索

它不是典型压缩标准，而更像描述层标准。

### MPEG-21

更偏向完整多媒体应用框架，关注：

- 内容标识
- 权限表达
- 更完整的应用体系

对日常开发来说接触没那么频繁，但它能帮助理解多媒体标准化并不只关心编解码。

## 7. 最值得记住的几条关系

如果只想建立工程上的整体感，先记住下面这些关系就够用了：

- MP3 <- MPEG-1 Layer 3
- AAC <- MPEG-2 / MPEG-4 音频体系
- MP4 <- MPEG-4 Part 14
- ISO Base Media File Format <- MPEG-4 Part 12
- AVC / H.264 <- MPEG-4 Part 10

一旦把这些连接起来，你再看文件格式、封装说明或标准文档时，就不会觉得它们像一堆互不相关的名词。

## 8. 总结

理解 MPEG 的价值，不在于背诵所有 Part 编号，而在于建立一条连续主线：

- MPEG 是重要的音视频标准来源
- MPEG-1 / 2 / 4 是工程里最常见的主干
- MP3、AAC、MP4、H.264 都不是孤立名词，而是这条主线上的节点

这条线一旦建立起来，再去看容器、编码器、播放器支持和文件结构，很多原本零散的概念会自然连起来。
