...
ARM 启动问题最麻烦的阶段，往往是串口还没起来的时候。没有日志并不能说明 CPU 没跑，也可能是启动介质、时钟、DDR 或 UART 初始化中的任一步停住了。
...
每一棒都留一个能观测的标记：寄存器、GPIO、串口字符或时间戳都可以。先证明上一阶段完成，再查下一阶段；在最早日志都没出现时直接猜内核参数，通常方向就错了。
