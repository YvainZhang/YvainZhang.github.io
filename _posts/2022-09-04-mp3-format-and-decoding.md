...
做 MP3 解析时，我习惯先把文件切成两块看：标签和音频帧。播放器能显示歌名，不代表帧一定能正常解码；同样，音频能播，也不代表时长、封面和 VBR 信息处理对了。
...
如果标签正常、播放失败，就别在 ID3 上继续耗时间；如果能播但时间轴不对，先看 VBR 信息和帧长估算。把文件层和解码层分开，能少走不少弯路。
