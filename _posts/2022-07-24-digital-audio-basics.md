...
最早整理音频资料时，我把采样率、位深、PCM、WAV、AAC、MP3 混在同一页里，越记越乱。后来才发现，这些词根本不在同一层：有的是参数，有的是数据表示，有的是编码，还有的是容器。

这篇就按我后来使用的分层方式重新整理一遍。先把位置摆对，公式和处理流程反而不难。
...
拿到一个音频问题，我一般先问四句：输入参数是什么，数据是不是 PCM，外面套了什么容器，里面用了什么编码。播放链路再长，也能沿着这四句往下拆。

如果只记一个习惯，就记得别用文件后缀代替分析。后缀只能给线索，真正有用的是流信息和实际数据。
