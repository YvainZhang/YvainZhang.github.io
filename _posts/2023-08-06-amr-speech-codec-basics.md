...
AMR 通常出现在通话、录音和移动设备的语音链路里。分析它之前要先放下音乐编码那套预期：它优先考虑的是语音可懂度、码率和链路适应能力，不是高保真。
...
抓包里的 AMR 和磁盘上的 `.amr` 文件不一定采用同一种打包方式，别拿文件头解析逻辑直接套网络帧。NB / WB、mode 和 IF1 / IF2 先确认清楚，再谈解码。
