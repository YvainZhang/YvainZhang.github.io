...
会写几条 FFmpeg 命令，不等于真正把 FFmpeg 用稳了。工程里更常见的难点其实有两个：

1. 你装的 FFmpeg 到底支持哪些能力
2. 面对不同任务，应该选拷贝、转码、裁剪、分片还是批处理

这篇文章把重点放在“工具链怎么落地”，而不只是记几条命令。
...
真正值得重视的是 `configure` 阶段，因为它决定了最终 FFmpeg 具备什么能力。
...
FFmpeg 的工程使用可以简化成一句话：

**先明确目标，再决定是拷贝、转码、抽流、重采样还是分片。**

安装只是起点，真正拉开差距的是：你是否清楚自己手里的 FFmpeg 支持什么，以及面对任务时是否能选对处理方式。
