...
这份内存管理笔记改过很多次。单独记页表、VMA、伙伴系统、swap 和 OOM，总是过一阵又散掉；按一次内存访问和一次内存紧张的过程串起来，反而容易记。
...
`free` 里“剩余内存少”不等于泄漏，进程虚拟地址大也不等于物理内存都被占了。要结合 RSS、page cache、slab、回收和随时间的趋势一起看。
