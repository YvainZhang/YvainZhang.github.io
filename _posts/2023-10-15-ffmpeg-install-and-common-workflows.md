...
项目里用 FFmpeg，我先确认两个问题：

1. 你装的 FFmpeg 到底支持哪些能力
2. 面对不同任务，应该选拷贝、转码、裁剪、分片还是批处理

命令本身一般不难，麻烦的是网上找到的命令未必适合当前编译配置，也未必符合这次输入文件。
...
先保存 `ffmpeg -version`、完整命令和完整控制台输出。只截最后一行 error，通常会把真正的输入信息丢掉。

再确认这次是拷贝、转码、抽流、重采样还是分片。目标没说清楚，命令越改越长也解决不了问题。
